...
test("getOfficeUrl prints the default React dashboard URL for staff fallback instructions", () => {
  assert.equal(getOfficeUrl({}), "http://localhost:3000/dashboard");
});

test("getOfficeUrl follows a configured APP_PORT", () => {
  assert.equal(getOfficeUrl({ APP_PORT: "3198" }), "http://localhost:3198/dashboard");
});
