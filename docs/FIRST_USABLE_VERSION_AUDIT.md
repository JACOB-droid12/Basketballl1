...
| Staff-friendly launcher | `START-HERE.bat`; `backup-database.bat`; `README-FIRST-WINDOWS.txt`; `docs/OFFLINE_INSTALL_CHECKLIST.md`; `tests/oneClickSetup.test.js`; `npm run verify:bundle` | Implemented and tested |
...
| Prepared offline folder exists | `dist\barangay-court-scheduler-offline` contains `node_modules`, SQL files including `setup.sql`, `backup-database.bat`, `setup-database-only.bat`, app source, views, CSS, and setup/start scripts | Verified locally |
