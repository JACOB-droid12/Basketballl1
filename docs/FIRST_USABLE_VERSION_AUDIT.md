...
Build and harden an offline Windows local web application for the Barangay Sto. Niño Basketball Court Scheduling System. The first usable version should keep the supplied HTML prototype as the visible staff interface, run the backend and MySQL/MariaDB database locally, let authorized barangay personnel log in, manage reservations locally, prevent double bookings, view available and reserved slots, update reservation statuses, maintain account/activity records, and separate ordinary staff daily startup from installer/admin maintenance tools.
...
| Backup/restore commands | `scripts/backup-mysql.mjs`; `scripts/restore-mysql.mjs`; `backup-database.bat`; backup/restore tests; disposable Oracle MySQL and MariaDB client tools verified live backup/restore | Implemented and live-tested on disposable local Oracle MySQL and MariaDB |
...
| Staff-friendly launcher | `START-HERE.bat`; `STAFF-DAILY-USE.txt`; `backup-database.bat`; `create-desktop-shortcut.bat`; `scripts/create-desktop-shortcut.ps1`; `README-FIRST-WINDOWS.txt`; `docs/OFFLINE_INSTALL_CHECKLIST.md`; `tests/oneClickSetup.test.js`; `npm run verify:bundle` | Implemented and tested |
| Separated daily and maintenance shortcuts | `scripts/create-desktop-shortcut.ps1` creates `Barangay Court Scheduler` for `start-barangay-office.bat` and `Barangay Court Scheduler - Maintenance` for `START-HERE.bat`; `tests/oneClickSetup.test.js` runs safe `-WhatIf` verification without creating real shortcuts | Implemented and tested |
...
| Office deployment sign-off report | `run-office-signoff.bat`; `scripts/run-office-signoff.ps1`; `reports\office-signoff` output; `tests/oneClickSetup.test.js`; `docs/DEPLOYMENT_GUIDE.md`; includes `npm run verify:offline-runtime` before backup | Implemented and tested; requires actual office MySQL/MariaDB for passing sign-off |
...
| Ordinary staff daily-use guide | `STAFF-DAILY-USE.txt`; `START-HERE.bat` opens it through `Open quick instructions`; `tests/oneClickSetup.test.js`; `tests/offlineBundle.test.js`; `npm run verify:bundle` | Implemented and tested |
...
| Browser opens after server readiness | `src/serverStartup.js` opens the browser from the `app.listen` callback only when `OPEN_BROWSER=1`; `start-barangay-office.bat` sets that flag; `tests/serverStartup.test.js` and `tests/oneClickSetup.test.js` cover the behavior | Implemented and tested |
| Staff fallback URL follows configured port | `scripts/print-office-url.mjs`; `start-barangay-office.bat`; `tests/officeUrlPrinter.test.js`; docs tell staff to use the address shown in the startup window | Implemented and tested |
...
| Offline runtime verification | `scripts/verify-offline-runtime.mjs`; `npm run verify:offline-runtime`; `tests/offlineRuntimeVerifier.test.js`; `README.md`; `docs/DEPLOYMENT_GUIDE.md` | Implemented and tested |
| Prepared offline folder exists | `dist\barangay-court-scheduler-offline` contains `node_modules`, SQL files including `setup.sql`, `backup-database.bat`, `create-desktop-shortcut.bat`, `setup-database-only.bat`, app source, views, CSS, and setup/start scripts | Verified locally |
...
| Prototype loads without CDN dependency | `public/prototype/sto-nino-court-reservation-system-prototype.html` uses local vendor scripts and no Google Fonts import; `src/features/prototype/prototypeRoutes.js` still rewrites old CDN script/font references defensively; bundle verifier requires both vendor files | Implemented and browser-verified |
...
- Staff daily-use guide on 2026-05-11: `STAFF-DAILY-USE.txt` was added as a root-level plain guide for ordinary barangay staff, `START-HERE.bat` now opens it from `Open quick instructions` with a fallback to `README-FIRST-WINDOWS.txt`, and the foundation/offline bundle verifiers require it in the prepared folder.
- Prototype offline resource hardening on 2026-05-11: the copied HTML prototype no longer imports Google Fonts or CDN script URLs, while `prototypeRoutes` still strips/rewrites those external references defensively. Focused tests passed with `npm test -- tests\prototypeRoutes.test.js tests\oneClickSetup.test.js tests\offlineBundle.test.js` at 21/21 tests, and Playwright browser verification loaded `/prototype` at `http://127.0.0.1:3210/prototype` with only same-origin local resources.
- Offline runtime verifier on 2026-05-11: `scripts/verify-offline-runtime.mjs` and `npm run verify:offline-runtime` were added so the project can repeatedly start the local app on a temporary port, check `/health`, fetch `/prototype`, and fail on external `http://` or `https://` resource references. Focused tests passed with `npm test -- tests\offlineRuntimeVerifier.test.js tests\offlineBundle.test.js` at 8/8 tests, `npm run verify:offline-runtime` passed at `http://127.0.0.1:53451/prototype`, `npm test` passed with 146/146 tests, and `npm run verify:bundle` confirmed the new script is included in the prepared offline folder. Chrome DevTools also loaded `http://127.0.0.1:3210/prototype/`, rendered the prototype login screen, confirmed backend JSON from `/health` and `/api/prototype/session`, and showed no external HTTP resources.
- Port-aware startup verification on 2026-05-11: `npm test -- tests\officeUrlPrinter.test.js tests\oneClickSetup.test.js tests\offlineBundle.test.js` passed with 18/18 tests, `node --check scripts\print-office-url.mjs` passed, `node scripts\print-office-url.mjs` printed `http://localhost:3000/prototype`, `APP_PORT=3199 node scripts\print-office-url.mjs` printed `http://localhost:3199/prototype`, `npm run verify:sql`, `npm run verify:ui`, `npm run verify:foundation`, and full `npm test` passed with 142/142 tests, and headless Chrome loaded `/prototype` from a temporary custom-port local server after `/health` was ready.
- Duplicate daily-start hardening on 2026-05-11: `src/serverStartup.js` now handles `EADDRINUSE` by checking `http://localhost:<port>/health`; when the Barangay Court Scheduler is already running and `OPEN_BROWSER=1`, it opens the existing `/prototype` page and exits cleanly, while a non-scheduler port conflict gets a clear support message. `TROUBLESHOOT-WINDOWS.txt`, `README-FIRST-WINDOWS.txt`, and `docs/DEPLOYMENT_GUIDE.md` document the behavior.
- Duplicate daily-start verification on 2026-05-11: `npm test -- tests\serverStartup.test.js tests\oneClickSetup.test.js` passed with 19/19 tests, `node --check src\serverStartup.js`, `node --check src\server.js`, `npm run verify:sql`, `npm run verify:ui`, and full `npm test` passed with 139/139 tests. Headless Chrome loaded `/prototype` from a temporary local server after `/health` was ready, and the temporary server was stopped. `npm run verify:foundation`, `npm run bundle:offline`, and `npm run verify:bundle` passed, confirming the updated startup files and docs are in the prepared offline folder.
- Startup readiness improvement on 2026-05-11: `src/serverStartup.js` now opens the office browser from the Express `listen` callback only when `OPEN_BROWSER=1`, and `start-barangay-office.bat` sets that flag for daily shortcut startup. Focused verification passed with `npm test -- tests\serverStartup.test.js tests\oneClickSetup.test.js` at 15/15 tests, `node --check src\serverStartup.js`, and `node --check src\server.js`.
- Startup readiness browser check on 2026-05-11: headless Chrome loaded `http://127.0.0.1:3198/prototype` from a temporary local Node server after `/health` reported ready, found the prototype login text, and the temporary server was stopped afterward.
- Startup readiness package verification on 2026-05-11: `npm run verify:sql`, `npm run verify:ui`, and full `npm test` passed with 135/135 tests; after adding `src/serverStartup.js` to foundation/bundle manifests, `npm test -- tests\offlineBundle.test.js tests\serverStartup.test.js tests\oneClickSetup.test.js` passed with 20/20 tests, `npm run verify:foundation` passed, `npm run bundle:offline` refreshed the prepared folder, and `npm run verify:bundle` confirmed `src/serverStartup.js` is included.
- Staff shortcut split on 2026-05-11: `create-desktop-shortcut.bat` and `scripts/create-desktop-shortcut.ps1` now create two shortcuts, `Barangay Court Scheduler` for direct daily startup through `start-barangay-office.bat` and `Barangay Court Scheduler - Maintenance` for `START-HERE.bat`; `npm test -- tests\oneClickSetup.test.js` passed with 10/10 tests including a real `-WhatIf` run against a temporary Desktop folder that verified no `.lnk` files were created.
- Revised offline deployment goal on 2026-05-11: `README.md`, `README-FIRST-WINDOWS.txt`, `docs/DEPLOYMENT_GUIDE.md`, `docs/OFFLINE_INSTALL_CHECKLIST.md`, and `docs/CODEX_HANDOFF.md` now describe the Windows-only offline local web app goal, prototype-as-frontend direction, local Node/MySQL backend, direct daily staff shortcut, and separate maintenance launcher.
- Current shortcut/deployment verification on 2026-05-11: `npm test -- tests\offlineBundle.test.js tests\oneClickSetup.test.js` passed with 15/15 focused tests, `cmd /c "echo 9|START-HERE.bat"` confirmed the menu opens and exits with `Create desktop shortcuts`, `npm run verify:foundation` passed, `npm run bundle:offline` refreshed `dist\barangay-court-scheduler-offline`, `npm run verify:bundle` passed and confirmed `create-desktop-shortcut.bat` plus `scripts/create-desktop-shortcut.ps1` are included, `npm run verify:sql` passed, `npm run verify:ui` passed for 15 office screens, and full `npm test` passed with 130/130 tests.
...
- Office sign-off checklist hardening on 2026-05-10: `scripts/run-office-signoff.ps1` now writes explicit report fields for the actual MySQL/MariaDB service version, MySQL client tools, office browser, printer name, readable printed output, and barangay personnel sign-off; `npm test -- tests\oneClickSetup.test.js tests\offlineBundle.test.js`, PowerShell parser check, `npm run bundle:offline`, `npm run verify:bundle`, and `git diff --check` passed.
- First-run guide sign-off note on 2026-05-10: `README-FIRST-WINDOWS.txt` now tells staff that `run-office-signoff.bat` asks for the actual MySQL/MariaDB version, browser, printer, readable print output, and barangay personnel sign-off; `npm test -- tests\offlineBundle.test.js`, `npm run bundle:offline`, `npm run verify:bundle`, and `git diff --check` passed.
